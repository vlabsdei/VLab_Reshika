...
const hudInclineAngle = document.getElementById('hudInclineAngle');
...
function registerEvents() {
  ...
  const terrainSelect = document.getElementById('terrainSelect');
  if (terrainSelect) {
    terrainSelect.addEventListener('change', updateMilestone1Calculations);
  }
...
function endSimulationTest() {
  ...
  const inputBrakeFreq = document.getElementById('inputBrakeFreq');
  if (inputBrakeFreq) inputBrakeFreq.disabled = false;
...
function runPhysicsStep() {
  ...
  if (activeProfile === 'eco') {
    ...
    numBrakes = Math.max(1, numBrakes); // ensure at least 1 cycle
    
    let cycleDuration = 30.0 / numBrakes;
    let cycleIdx = Math.floor(t / cycleDuration);
    if (cycleIdx >= numBrakes) cycleIdx = numBrakes - 1; // clamp to last cycle
    ...
    let t_cruise = cycleDuration * 0.60;
    ...
    // aggressive profile calculations
    let numBrakes = Math.round(brakeFreq * 0.5);
    ...
    if (numBrakes >= 1) {
      ...
      for (let i = 1; i <= numBrakes; i++) {
        let bStart = i * eventInterval - 1.0;
        let bEnd = bStart + 2.0;
        if (t >= bStart && t < bEnd) {
          inBrakePeriod = true;
          break;
    ...
    if (inBrakePeriod) {
      a_target = -10.0; // harsh deceleration
      v_target = 0; // complete stop
      isBraking = true;
    } else {
      if (vActual < v_max) {
  ...
  batteryPercent = 100.0 - ((energyConsumed / BATTERY_CAPACITY_WH) * 100.0);
...
function getVerdictAndScore() {
  ...
  // Rule: any driving profile, if crosses the speed range on 110, must show verdict as Fail with dynamic AI score
  if (safeSpeed >= 110) {
    verdict = 'Fail';
  } else {
...
function animate() {
  ...
  if (roadElement) {
    ...
    roadElement.style.transform = `rotateX(${roadTiltDeg}deg) translateZ(-40px)`;
  }
  ...
  // update hud inclines
  if (hudGridAxis && hudInclineAngle) {
  ...
  if (velocity > 0.05) {
    // Cap visual velocity to avoid stroboscopic/aliasing effects (e.g. backward scrolling at high speeds)
    // Compress speed above 40 m/s (144 km/h) using a soft curve
    let visualVelocity = velocity;
    if (visualVelocity > 30) {
      visualVelocity = 30 + (velocity - 30) * 0.4; // Less compression to scale higher speed visibility properly
    }

    wheelRotationAngle = (wheelRotationAngle + visualVelocity * 8.5) % 360; // spin wheels
    
    // Apply dynamic wheel spin blur based on velocity
    let wheelBlur = Math.min(4, velocity * 0.035);
    ...
    // scroll road stripes
    roadScrollPosition = (roadScrollPosition + visualVelocity * 60 * 0.016) % 1000;
  ...
  stripes.forEach((stripe, idx) => {
    ...
    let stretch = Math.min(45, velocity * 0.35);
    stripe.style.height = `${45 + stretch}px`;
    
    let blurVal = Math.min(5, velocity * 0.04);
